...
const Loginpage = () => {
  ...
  //   console.log(value)
  //   axios
  //     .post("http://localhost:8080/api/employee/login", {email:value?.email[0],password:value?.password[0]})
  ...
  const handleSubmit = (e) => {
    ...
    axios.post("http://localhost:8080/api/employee/login", {email:value?.email[0],password:value?.password[0]})
      .then((res) => {
        console.log('res',res)
        if (res.data.token) {
          // console.log(res,"login")
